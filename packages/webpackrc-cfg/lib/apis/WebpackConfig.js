 ...
 * 用于获取 webpack 配置
 */
let WebpackConfig = (_temp = _class = class WebpackConfig {
  constructor() {
    this.options = {};
  }
  ...
  set output(output = {}) {
    ...
    if (output.path) {
      output.path = _path2.default.resolve(this.context, output.path);
    };
...
}, _class.loaders = _loaders2.default, _class.plugins = _plugins2.default, _temp);
