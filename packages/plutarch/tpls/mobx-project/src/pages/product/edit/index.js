...
import { EditUnits as Attributes } from '../_components/Attributes';
  ...
  componentDidMount(){
    ...
    if ( id )
      product.getProduct(id).then(res => {
        ...
        Promise.all([
          product.getCategoryByLevels(cids),
          ...
          <Form>
            ...
            <FormItem {...formItemLayout} label={$i18n('model.product.categories')}>
              {getFieldDecorator('cids', {
                ...
                <CategoryCascader 
                  placeholder={`${$i18n('text.please_input')}${$i18n('model.product.categories')}`} 
